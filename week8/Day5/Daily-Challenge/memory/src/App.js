...
function App() {
  ...
  const handleCardClick = (id) => {
    ...
    const superhero = newSuperheroes[0].superheroes.find(hero => hero.id === id);

    if (superhero.clicked) {
      setScore(0);
      setSuperheroes([{ superheroes: newSuperheroes[0].superheroes.map(hero => ({ ...hero, clicked: false })) }]);
    } else {
      setScore(score + 1);
      superhero.clicked = true;
      setSuperheroes([{ superheroes: shuffle(newSuperheroes[0].superheroes) }]);
    }
  };
  ...
  return (
    <div className="App">
      <header className="App-header">
        <div style={{ margin: '10px', fontSize: '24px' }}>Score: {score}</div>
        <div style={{ margin: '10px', fontSize: '24px' }}>Top Score: {topScore}</div>
      </header>
      <div className="superheroes-container">
        {superheroes[0].superheroes.map((hero) => (
          <div key={hero.id} onClick={() => handleCardClick(hero.id)} className="hero-card">
